...
const char * const BITCOIN_CONF_FILENAME = "resq.conf";
const char * const BITCOIN_PID_FILENAME = "resqd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "resq" is a composite category enabling all Resq-related debug output
            if(ptrCategory->count(string("resq"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ResqCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ResqCore
    // Mac: ~/Library/Application Support/ResqCore
    // Unix: ~/.resqcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ResqCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ResqCore";
#else
    // Unix
    return pathRet / ".resqcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty resq.conf if it does not excist
...
//        	ReadConfigFile(mapSettingsRet, mapMultiSettingsRet);
        	...
        	LogPrintf("resq.conf file not found or can't be created\n");
    ...
    {
        // Don't overwrite existing settings so command line settings override resq.conf
