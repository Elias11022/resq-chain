...
pkgdatadir = $(datadir)/resqcore
pkgincludedir = $(includedir)/resqcore
pkglibdir = $(libdir)/resqcore
pkglibexecdir = $(libexecdir)/resqcore
...
CONFIG_HEADER = $(top_builddir)/src/config/resq-config.h
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/resq-config.h.in \
...
ACLOCAL = ${SHELL} /home/paulh/resq/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
ARFLAGS = cr
AUTOCONF = ${SHELL} /home/paulh/resq/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /home/paulh/resq/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /home/paulh/resq/build-aux/missing automake-1.15
AWK = gawk
BDB_CFLAGS =
BDB_CPPFLAGS =
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = resq-cli
BITCOIN_DAEMON_NAME = resqd
BITCOIN_GUI_NAME = resq-qt
BITCOIN_TX_NAME = resq-tx
...
COPYRIGHT_HOLDERS_FINAL = The Resq Core developers
COPYRIGHT_HOLDERS_SUBSTITUTION = Resq Core
COPYRIGHT_YEAR = 2018
CPP = gcc -E
...
MAINT =
MAKEINFO = ${SHELL} /home/paulh/resq/build-aux/missing makeinfo
MAKENSIS =
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS =
...
OTOOL64 =
PACKAGE = resqcore
PACKAGE_BUGREPORT = https://github.com/NicholasAdmin/Resq/issues
PACKAGE_NAME = Resq Core
PACKAGE_STRING = Resq Core 1.0.3
PACKAGE_TARNAME = resqcore
PACKAGE_URL =
...
ZMQ_CFLAGS =
ZMQ_LIBS = -lzmq
abs_builddir = /home/paulh/resq
abs_srcdir = /home/paulh/resq
abs_top_builddir = /home/paulh/resq
abs_top_srcdir = /home/paulh/resq
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN =
...
install_sh = ${SHELL} /home/paulh/resq/build-aux/install-sh
...
BITCOIND_BIN = $(top_builddir)/src/resqd$(EXEEXT)
BITCOIN_QT_BIN = $(top_builddir)/src/qt/resq-qt$(EXEEXT)
BITCOIN_CLI_BIN = $(top_builddir)/src/resq-cli$(EXEEXT)
BITCOIN_WIN_INSTALLER = $(PACKAGE)-$(PACKAGE_VERSION)-win$(WINDOWS_BITS)-setup$(EXEEXT)
OSX_APP = Resq-Qt.app
OSX_DMG = Resq-Core.dmg
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_resq_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info rpc_test.info rpc_test_filtered.info \
  leveldb_baseline_filtered.info test_resq_coverage.info test_resq.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/Resq-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/resq-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/resq-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/resq-config.h
$(top_srcdir)/src/config/resq-config.h.in:  $(am__configure_deps)
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/resq-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/Resq-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Resq-Qt

$(OSX_DMG): $(APP_DIST_EXTRAS)
	$(GENISOIMAGE) -no-cache-inodes -D -l -probe -V "Resq-Core" -no-pad -r -dir-mode 0755 -apple -o $@ dist
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Resq-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_resq.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_resq -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_resq_filtered.info: test_resq.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_resq_filtered.info
#	$(MKDIR_P) qa/tmp
...
#rpc_test.info: test_resq_filtered.info
...
#test_resq_coverage.info: baseline_filtered_combined.info test_resq_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_resq_filtered.info -o $@

#total_coverage.info: baseline_filtered_combined.info test_resq_filtered.info block_test_filtered.info rpc_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_resq_filtered.info -a block_test_filtered.info -a rpc_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_resq.coverage/.dirstamp:  test_resq_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_resq.coverage/.dirstamp total.coverage/.dirstamp

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf coverage_percent.txt test_resq.coverage/ total.coverage/ qa/tmp/ cache/ $(OSX_APP)
